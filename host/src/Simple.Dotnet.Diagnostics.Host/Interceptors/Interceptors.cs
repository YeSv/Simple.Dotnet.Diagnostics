...
public static class Interceptors
{
    ...
    public static IInterceptor[] Load(in LoadInterceptorsCommand command)
    {
        ...
        using var rent = BufferWriterPool<IInterceptor>.Shared.Get();
        foreach (var interceptorDir in interceptorsDirs)
        {
            ...
            foreach (var interceptor in interceptors ?? Array.Empty<IInterceptor>()) rent.Value.Append(interceptor);
        }

        return rent.Value.WrittenSpan.ToArray();
    }
    ...
    static UniResult<Assembly?, Exception> GetAssemblyFromContext(string interceptorDir)
    {
        try
        {
            var dllName = Path.GetFileName(interceptorDir) ?? Path.GetDirectoryName(interceptorDir);
