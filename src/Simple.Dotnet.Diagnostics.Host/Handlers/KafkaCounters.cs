...
public sealed class KafkaCounters
{
        ...
        ILogger logger)
    {
        ...
        logger.LogWarning(registerResult.Error, "Failed to register kafka action, exception occurred");
        var response = ResponseMapper.ToResponse(registerResult, e => new(ErrorCodes.KafkaCountersFailed, "Failed to register an action"));
