...
public static class ResponseMapper
{
    public static Response<TOk> ToResponse<TOk, TError>(
        in Utilities.Results.UniResult<TOk, TError> result, 
    ...
    public static Response<TOk> ToResponse<TOk, TError>(in Utilities.Results.Result<TOk, TError> result, Func<TError, Error>? errorMapper) => result switch
    {
        { IsOk: true } => new(true, result.Ok!, default),
        _ => new(false, default, errorMapper!(result.Error!))
    };
